...
# backs up file but only if sourcefile and destfile are different files
...
def backupfile(sourcefile,destfile):
	if not os.path.exists(destfile) or not filecmp.cmp(sourcefile, destfile):
		try:
			...
			print("to: " + destfile)
			log("ERROR: Could NOT copy file " + sourcefile + \
               " to " + destfile, logfile)
...
logfile="logfile-"+str(now.month)+'-'+str(now.day)+'-'+str(now.year)+'-'+str(now.hour)+\
'-'+str(now.minute)+'-'+str(now.second)+'.txt'

# Intialize logfile
with open(logfile,"w") as f:
	f.write("PYTHON BACKUP LOGFILE\n")
	f.write("Backup started at: "+str(now.month)+str(now.day)+str(now.year)+str(now.hour)+ \
...
print("\nBackup complete!  View ", logfile, " for details.")

now=datetime.datetime.now()
log("\nBackup completed successfully on "+str(now.month)+str(now.day)+str(now.year)+ " at "+\
 str(now.hour)+':'+str(now.minute)+':'+str(now.second),logfile)